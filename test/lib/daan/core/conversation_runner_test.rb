...
class Daan::Core::ConversationRunnerTest < ActiveSupport::TestCase
  ...
  test "transitions to completed" do
    with_stub_step { Daan::Core::ConversationRunner.call(@chat) }
  ...
  test "transitions to blocked when max_steps reached and agent wants to continue" do
    ...
    with_stub_tool_step { Daan::Core::ConversationRunner.call(@chat) }
  ...
  test "sets Thread.current[:daan_active_hooks] during RunStep execution" do
    ...
    Daan::Core::Chats::ConfigureLlm.stub(:call, nil) do
      Daan::Core::Chats::RunStep.stub(:call, ->(chat, **) {
  ...
  test "clears Thread.current[:daan_active_hooks] after RunStep" do
    Daan::Core::Chats::ConfigureLlm.stub(:call, nil) do
      Daan::Core::Chats::RunStep.stub(:call, ->(*) {
        OpenStruct.new("tool_call?" => false, role: "assistant")
      }) do
        Daan::Core::ConversationRunner.call(@chat)
      end
  ...
  test "clears Thread.current[:daan_active_hooks] even when RunStep raises" do
    Daan::Core::Chats::ConfigureLlm.stub(:call, nil) do
      Daan::Core::Chats::RunStep.stub(:call, ->(*) { raise RuntimeError, "boom" }) do
        assert_raises(RuntimeError) { Daan::Core::ConversationRunner.call(@chat) }
      end
    end
    assert_nil Thread.current[:daan_active_hooks]
