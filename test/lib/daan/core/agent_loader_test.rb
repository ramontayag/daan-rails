...
class Daan::Core::AgentLoaderTest < ActiveSupport::TestCase
  setup do
  ...
  test "parse returns a hash with agent attributes" do
    ...
    assert_equal 16, definition[:max_steps]
    assert definition[:system_prompt].start_with?("You are the Chief of Staff")
  end

  test "sync! registers a Daan::Core::Agent for each definition file" do
    Daan::Core::AgentLoader.sync!(@definitions_path)

    agent = Daan::Core::AgentRegistry.find("chief_of_staff")
    assert_instance_of Daan::Core::Agent, agent
    assert_equal "Chief of Staff", agent.display_name
    assert_equal "claude-haiku-4-5-20251001", agent.model_name
  end

  test "sync! re-running overwrites previous registration" do
    Daan::Core::AgentLoader.sync!(@definitions_path)
    Daan::Core::AgentLoader.sync!(@definitions_path)
    assert_equal 5, Daan::Core::AgentRegistry.all.length # 5 agents: chief_of_staff, developer, engineering_manager, agent_resource_manager, ryan_singer
  end
  ...
  test "parse returns base_tools array for developer agent" do
    definition = Daan::Core::AgentLoader.parse(@definitions_path.join("developer.md"))
  ...
  test "resolve_includes replaces include directives with partial content" do
    ...
    result = Daan::Core::AgentLoader.resolve_includes(content, base_dir)
  ...
  test "loads engineering_manager with delegates_to developer" do
    Daan::Core::AgentLoader.sync!(@definitions_path)
    agent = Daan::Core::AgentRegistry.find("engineering_manager")
  ...
  test "loads agent_resource_manager with appropriate tools and workspace" do
    Daan::Core::AgentLoader.sync!(@definitions_path)
    agent = Daan::Core::AgentRegistry.find("agent_resource_manager")
    assert_not_nil agent
  ...
  test "parses hooks from frontmatter" do
    Dir.mktmpdir do |dir|
      ...
      File.write(path, <<~MD)
        ...
        You shape.
      MD
      definition = Daan::Core::AgentLoader.parse(path)
  ...
  test "agents with Bash tool include repo exploration instructions" do
    ...
    Daan::Core::AgentRegistry.all.each do |agent|
