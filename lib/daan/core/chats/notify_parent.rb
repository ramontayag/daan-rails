    ...
    class NotifyParent
      def self.on_completion(chat)
        ...
        unless agent_reported_back?(chat, agent)
          ...
          Daan::Core::CreateMessage.call(
            chat.parent_chat,
            role: "user",
            content: "#{Daan::Core::SystemTag::PREFIX} #{agent.display_name} completed their task without calling report_back. " \
      ...
      def self.on_termination(chat, status)
        ...
        Daan::Core::CreateMessage.call(
          chat.parent_chat,
          role: "user",
          content: "#{Daan::Core::SystemTag::PREFIX} #{agent.display_name}'s thread is now #{status}. " \
      ...
      def self.agent_reported_back?(chat, agent)
        ...
        chat.parent_chat.messages
            ...
            .where_content_like("#{Daan::Core::SystemTag::PREFIX} #{agent.display_name} reported back: %")
            .exists?
      end
