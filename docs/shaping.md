...
| Part | Mechanism |
|------|-----------|
| **A1** | **Core data model** — Agent (name, status — in DB for querying), Thread (pair of participants, task reference, per-thread LLM lock via Solid Queue concurrency controls), Message (thread, sender, sender_type, content, message_type [text/tool_call/tool_result/summary], metadata JSON with token_count), Task (state machine [pending/in_progress/completed/failed/blocked], parent_task reference, turn_count, assigned_agent), Memory (content, type [concept/fact/skill/experience], embedding vector via neighbor gem). Agent config (model, max_turns, workspace, delegates_to, tools) lives in-memory from parsed definition files, not in DB. |
| **A2** | **Agent loader** — Reads frontmatter markdown files from `lib/daan/core/agents/` and `config/agents/` (overrides). Parses YAML front-matter into agent config. Registers agents at boot (production) or per-request (development). Deployment overrides take precedence (same-name file). Agent DB records created/synced on load for status tracking. |
