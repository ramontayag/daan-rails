...
### D6: Memory
Powered by the `swarm_memory` gem (filesystem-based, not DB-backed). A single shared storage directory (`storage/memory/`) is used by all agents. Storage is instantiated once at boot via a Rails initializer and injected into tools alongside `workspace:` and `chat:`. Four memory types: concept, fact, skill, experience. Metadata includes `confidence:` (high/medium/low), `tags:`, `title:`, `domain:`, and `related:` for relationship linking. Hybrid search: 50% semantic (ONNX embeddings via `informers`) + 50% keyword (tag matching). No approval needed — memories are internal state. Memory tools: SwarmMemory's built-in `MemoryWrite`, `MemoryRead`, `MemoryEdit`, `MemoryDelete`, `MemoryGlob`, `MemoryGrep` (no custom memory tool code needed). `MemoryEdit` does exact string replacement within a file — agents correct memories without overwriting the whole entry. Auto-retrieval: `ConversationRunner` calls `storage.semantic_index.search` on the last user message and injects results into the agent's system prompt.
...
### D11: SwarmMemory Dependency (Revised)
Daan uses `swarm_memory` for its memory layer. `swarm_sdk` is a transitive dependency of `swarm_memory` but Daan does not use SwarmSDK's agent orchestration, tool configurator, or plugin system — only the storage layer (`SwarmMemory::Core::Storage`, `SwarmMemory::Adapters::FilesystemAdapter`, `SwarmMemory::Embeddings::InformersEmbedder`) and tools (`MemoryWrite`, `MemoryRead`, `MemoryEdit`, `MemoryDelete`, `MemoryGlob`, `MemoryGrep`). SwarmSDK's plugin registration (`SwarmMemory::Integration::SDKPlugin`) is not used.
...
### D40: Memory Consolidation on Task Completion
When a task transitions to `completed`, the `ConversationRunner` injects a final system nudge prompting the agent to save anything worth remembering before the thread closes. This is a lightweight complement to compaction (D23) — compaction fires mid-thread when context is full, D40 fires at natural task boundaries regardless of thread length. Both write to the same shared memory store.
...
| **A3** | **Job chain execution** — Heartbeat rule (D29): any new message in an idle thread enqueues an LLM Job. LLM Job: load thread context (summary + recent messages + relevant memories via semantic search on last message), call RubyLLM, save response as Message with token metadata. If tool call(s) → enqueue Tool Job(s). Tool Job: execute tool, save result as Message, post triggers new LLM Job (D29). Per-thread lock via Solid Queue concurrency_key. Turn counter on Task increments per LLM Job, enforces max_turns. Compaction check at LLM Job start (D23/D34). |
| **A4** | **Tool system** — Tools are `RubyLLM::Tool` subclasses. Built-in Daan tools: `Read`, `Write`, `Bash`, `DelegateTask`, `ReportBack`. Memory tools: SwarmMemory's `MemoryWrite`, `MemoryRead`, `MemoryEdit`, `MemoryDelete`, `MemoryGlob`, `MemoryGrep` (injected with shared storage). Git tools (A9) allowed network/API access. Custom tools use full Ruby class names. `delegates_to` enforced by `DelegateTask`. |
...
| **A6** | **Memory system** — `swarm_memory` gem (D11). Single shared `SwarmMemory::Core::Storage` instance (filesystem adapter + informers ONNX embedder) mounted at `storage/memory/`. Instantiated once at boot via Rails initializer. Injected into memory tools alongside `workspace:` and `chat:`. Automatic retrieval: `ConversationRunner` calls `storage.semantic_index.search` on last user message, injects top results into system prompt. Hybrid search: 50% semantic + 50% keyword tag matching. Memory entries have `confidence:` (high/medium/low), `tags:`, `title:`, `related:` metadata. |
