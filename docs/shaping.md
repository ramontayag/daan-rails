...
| **A4** | **Tool system** — Base class `Daan::Core::Tool` with description + call interface. Built-in: `Read`, `Write`, `Bash`, `DelegateTask`, `ReportBack`, `WriteMemory`, `SearchMemory`. All filesystem tools scoped to agent's workspace directory. `Daan::Core::Bash`: accepts `commands` (array of command arrays, e.g. `[["git","add","-A"],["git","commit","-m","msg"]]`) and optional `path` (workspace-relative working directory). Checks each command's first element against `allowed_commands` from the agent definition — no shell, runs via `Open3.capture3`. Network-capable (not workspace-filesystem-scoped). `allowed_commands` is a YAML list in the agent definition (D13); `Agent#tools` passes it to every tool at instantiation. Custom tools use full Ruby class names. `delegates_to` enforced by `DelegateTask`. |
...
| **A9** | **Self-modification** — `Daan::Core::Bash` (A4) with `allowed_commands: [git, gh]` in Developer definition covers all git and GitHub CLI operations. No per-operation tool classes. `gh` authenticates via `GITHUB_TOKEN` env var. Agent workflow: `git clone` into workspace subdir → `git checkout -b` new branch → Write files (existing tool) → `git add -A` + `git commit` → `git push` → `gh pr create`. Each step is one `Bash` call; all appear as observable tool call blocks in the thread. Changes take effect on next deploy (D36). |
